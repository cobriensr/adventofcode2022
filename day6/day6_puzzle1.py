...
# you need to add a subroutine to the device that detects a start-of-packet marker in the
# datastream. In the protocol being used by the Elves, the start of a packet is indicated by a
...
# your subroutine should report the value 7, because the first start-of-packet marker is complete
...
def extract_datastream(file:str) -> list[str]:
    """extract buffer from datastream"""
    buffers = []
...
datastream = extract_datastream(file="./day6/day6_puzzle1_data.txt")

def is_unique_chars(check:list[str]):
    """check if all elements are unique"""
    # Counting frequency
    freq = Counter(check)
    if len(freq) == len(check):
        return True
    else:
        return False
...
def find_unique_characters(data:list[str]) -> int:
    ...
    for i, letter in enumerate(data):
        for alpha in letter:
            ...
            unique.append(alpha)
            if len(unique) == 4:
                if is_unique_chars(unique) is False:
