...
def extract_cleaning_assignments(file:str) -> list:
    """extracts the item lists from the rucksacks"""
    assignment_list = []
    with open(file, 'r', encoding='utf-8') as data_file:
        for line in data_file:
            splits = line.replace("\n","").split(',')
            assignment_list.append(splits)
        return assignment_list

assignments = extract_cleaning_assignments(file="./day4/day4_puzzle1_data.txt")

def write_assignments(lists:list) -> list:
    """write the assignment ranges for each elf"""
    ranges = []
    for item in lists:
        new = []
        for numbers in item:
...
written = write_assignments(lists=assignments)
...
def build_ranges(range_list: list) -> int:
    ...
    for item in range_list:
        ...
        result_1 = all(elem in path_1 for elem in path_2)
        result_2 = all(elem in path_2 for elem in path_1)
